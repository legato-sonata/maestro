...
gh codespace cp ./maestro.env remote:maestro.env --codespace $CODESPACE_ID || { echo "Fatal Error: Failed to copy config. Is the Codespace fully initialized and did you push your code?"; exit 1; }
...
gh codespace cp remote:output.mp4 ./demo-recording.mp4 --codespace $CODESPACE_ID || { echo "Fatal Error: Failed to download artifact. The video may not have been created."; exit 1; }
